...
**Precise meaning**: External systems that give AI agents persistent state across sessions. The term covers at least four distinct functions: **personalization** (remembering user preferences), **task coordination** (tracking work state and dependencies), **institutional knowledge** (accumulating domain expertise over time), and **context engineering** (managing what enters the model's active context). These are fundamentally different problems with different solutions. See [Weng's architectural framing](articles.md#llm-powered-autonomous-agents) for the canonical breakdown of memory as one of four core agent components, and the Hu et al. taxonomy (ICLR 2026) for the most rigorous academic classification (factual / experiential / working memory; token-level / parametric / latent forms; formation / evolution / retrieval dynamics).

**Common drift**: Used interchangeably for any form of agent state persistence — from a CLAUDE.md file read at session start, to a vector database doing semantic retrieval, to a temporal knowledge graph with bi-temporal fact tracking. A tool optimized for conversation personalization (storing user preferences) is described as solving the same problem as a task tracker (managing work state) or a knowledge graph (accumulating domain expertise). This conflation leads to mismatched tool choices.

**Why it matters**: The [agent memory landscape survey](../ai-generated/agent-memory-landscape.md) found that most tools emphasize one or two of the four functions and claim the others by implication. The [Price of Meaning](articles.md#the-price-of-meaning-why-every-semantic-memory-system-forgets) proves that all semantic memory systems face structural forgetting and false recall — a constraint that no tool acknowledges in its documentation. The term's breadth obscures both the real problem being solved and the theoretical limits of the solution.
