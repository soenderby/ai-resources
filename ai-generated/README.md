...
- `higher-level-agent-orchestration.md` — essay on the loop hierarchy (inner loop → Ralph → middle loop → harness loop → loop engineering → self-harness). Central thesis: outer loops relocate rather than remove human judgment, and a loop only manufactures capability its verifier can *completely* score — so the leverage is in the oracle, not the loop. Engages the self-play counterargument (cheap+total oracles create superhuman capability) and the compounding-vs-repeating distinction. Skeptical spine: MAST, Long-Horizon Task Mirage, Illusion of Multi-Agent Advantage, METR 50/80 gap, STOP, Darwin Gödel Machine. Critically reviewed.
