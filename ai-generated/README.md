...
- `cybernetics-calm-technology-agent-loops.md` — companion to the above (generated by Codex GPT-5, review-revised by Claude). Applies cybernetics (requisite variety, good-regulator theorem, Beer's VSM, algedonic signals, second-order) and calm technology (center/periphery attention routing) as a design lens for agent loops: a high-level loop is a control system coupled to a human attention system. Explicitly a design lens/checklist, not an empirical result; self-critical about metaphor laundering and the difficulty of measuring "variety."
